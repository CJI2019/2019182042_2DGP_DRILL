...
class WalkingState:
    ...
    def do(boy):
        ...
        boy.oldX,boy.oldY = boy.x , boy.y

        boy.x += boy.x_velocity * game_framework.frame_time
        boy.y += boy.y_velocity * game_framework.frame_time

        boy.x = clamp(25, boy.x, server.background.w - 1 - 25) # 25는 소년이 화면 밖으로 안나가도록
    ...
    def draw(boy):
        ...
        boy.bx, boy.by = sx,sy
        boy.font.draw(sx - 40, sy + 40, '(%d)' % (server.count), (255, 255, 0))
...
class Boy:
    ...
    def get_bb(self):
        return self.bx - 25, self.by - 50, self.bx + 25, self.by + 50
