...
class WALL:
    ...
    def Crash(self,player,x,y):
        # x + player.Right_Idle.w//2 와 y + player.Right_Idle.h//2 는 player x1 , y1 
        # 이 둘이 벽의 사각형 내에 있으면 막음. 총 4개의 점
        if (x - player.Right_Idle.w//2 > self.x1 and x - player.Right_Idle.w//2 < self.x2 and
        y + player.Right_Idle.h//2 < self.y1 and y + player.Right_Idle.h//2 > self.y2):
            x += 4
            print("충돌")
