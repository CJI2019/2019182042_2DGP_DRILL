...
class IDLE:
    ...
    def draw(self):
        if self.face_dir == 1:
            ...
            self.image.clip_draw(int(self.frame) * 183, 337, 180, 168, self.x, self.y)

        else:
            # self.image.clip_draw(int(self.frame) * 100, 200, 100, 100, self.x, self.y)
            # self.image.clip_draw(int(self.frame) * 183, 337, 180, 168, self.x, self.y)
            self.image.clip_composite_draw(int(self.frame) * 183, 337, 180, 168, 0 ,'h',self.x, self.y,180, 168)
...
class RUN:
    ...
    def do(self):
        ...
        if int(self.frame) < 5: frameH = 0
        elif int(self.frame) < 10: frameH = 1
        elif int(self.frame) < 14: frameH = 2
    ...
    def draw(self):
        if self.dir == 1:
            ...
            if frameH == 0:
                ...
                self.image.clip_draw((int(self.frame) - 5) * 183, 168, 180, 168, self.x, self.y)
                print('1')
            elif frameH == 2:
                self.image.clip_draw((int(self.frame) - 10)* 183, 0, 180, 168, self.x, self.y)
            ...
            if frameH == 0:
                ...
                self.image.clip_composite_draw((int(self.frame) - 5) * 183, 168, 180, 168, 0 ,'h',self.x, self.y,180, 168)
                print('1')
            elif frameH == 2:
                self.image.clip_composite_draw((int(self.frame) - 10)* 183, 0, 180, 168, 0 ,'h',self.x, self.y,180, 168)
...
RUN_SPEED_KPH = 100 # km/h 마라토너의 평속
...
class Boy:
    ...
    def __init__(self):
        ...
        self.font = load_font('ENCR10B.TTF', 16) # 16 point size
