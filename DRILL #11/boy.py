...
class AUTO_RUN:
    ...
    def exit(self):
        print('EXIT AUTO_RUN')
        # run 을 나가서 , idle 로 갈 떄, run의 방향을 알려줄 필요가 있다.
    ...
    def do(self):
        ...
        self.x += self.dir
        if self.x > 800 or self.x < 0: self.dir *= -1; self.face_dir *= -1
...
class IDLE:
    ...
    def do(self):
        ...
        if self.timer == 0 : # 시간이 경과하면,
            #이벤트를 발생시켜줘야한다. TIMER
            # self.q.insert(0,TIMER) # 객체지향프로그래밍 위배 , q를 직접 액세스 하고 있기 때문.
...
next_state = {
    AUTO_RUN: {AD:IDLE , RD:RUN , LD:RUN,RU:IDLE,LU:IDLE},
    SLEEP: {RD:RUN, LD:RUN, RU:RUN, LU:RUN, AD:IDLE},
...
class Boy:
    ...
    def handle_event(self,event): # 소년이 스스로 이벤트를 처리할수 있게..
        ...
        if (event.type,event.key) in key_event_table:
            ...
            self.add_event(key_event) # 변환된 내부 이벤트를 큐에 추가. (self.q에 추가하는 것)
    ...
    def update(self):
        ...
        if self.q: # q에 뭔가 들어 있다면
            event = self.q.pop() # 이벤트를 가져오고 (pop한 요소를 event에 넣음)
